...
def talker(threadName, delay):
    ...
    for i in range(len(point_list)):
        ...
        # Move to the calibration position, guide the user to place the block
        ...
        # Waiting for the user to place the block, the robot arm moves to other places without blocking the camera
        while(go_cali_pos == 0):
            r1. sleep()
        if(go_cali_pos == 0):
            return
        ...
        cnt = 0
        # Wait for the calibration data collection before proceeding to the next action
